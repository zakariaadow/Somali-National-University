...
const AdminFaculties = () => {
  ...
  const fetchFaculties = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/faculties?is_active=true&${params}`, {
        withCredentials: true
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingFaculty) {
        await axios.put(
          `${API_BASE_URL}/faculties/${editingFaculty.id}`,
          data,
          { withCredentials: true }
        );
      } else {
  ...
  const handleDelete = async (facultyId) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/faculties/${facultyId}`, { withCredentials: true });
      fetchFaculties();
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-3 text-gray-600">Loading faculties...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-gray-600">Manage all faculties</p>
            </div>
            <button
              onClick={() => {
                resetForm();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {faculties.length === 0 ? (
            ...
            faculties.map((faculty) => (
              <div key={faculty.id} className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition">
                ...
                <p className="text-sm text-gray-600 mb-2">{faculty.description}</p>
                <p className="text-sm text-gray-500">
                  <span className="font-medium">College:</span> {faculty.college_name}
                </p>
                {faculty.dean_name && (
                  <p className="text-sm text-gray-500">
                    <span className="font-medium">Dean:</span> {faculty.dean_name}
                  </p>
                )}
