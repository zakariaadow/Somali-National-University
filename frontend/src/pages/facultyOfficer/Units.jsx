    ...
    const fetchData = async () => {
      try {
        ...
        const [unitsRes, deptsRes] = await Promise.all([
          axios.get(`${API_BASE_URL}/faculty-officer/units`, { withCredentials: true }),
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <p className="text-gray-600">Manage units in your faculty</p>
            </div>
          ...
          {filteredUnits.map((unit) => (
            <div key={unit.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
              ...
              <div className="mt-4 flex gap-2">
                <Link to={`/faculty-officer/units/${unit.id}`} className="text-blue-600 hover:text-blue-900 text-sm">
                  View
                </Link>
                <Link to={`/faculty-officer/units/${unit.id}/lecturers`} className="text-green-600 hover:text-green-900 text-sm">
                  Lecturers
                </Link>
              </div>
            </div>
